////////////////////////////////////////////////////////////////////////////////
//
// Filename: 	MasterSim.java
//
// Project:	ROBOSIM, for learning how to control autonomoous FTC robots
//
// Purpose:	This is the master simulation file for the robot having only two.
//			powered wheels.  It contains the main() entry point for the two-tracked
//		simulator, and then splices the logic together between other components of
//		the simulation.
//
...
public class MasterSim extends Applet implements Runnable {
	Frame	m_win;
	boolean	m_done;
	RobotState	m_state;
	...
	public	MasterSim(RController ctrl) {
		setLayout(new BorderLayout());
		m_win = null;
		m_done = false;
		m_controller = ctrl;
		m_robot = new SimRobot();
		...
		try {
			...
			while(!m_done) {
				...
				if (inputs == null) {
					...
					System.out.println(String.format(" <<--- < %6.2f , %6.2f >", inputs.lp, inputs.rp));
	...
	public static void main(String[] args) {
		// RController	ctl = new StudentController();
		// RController	ctl = new StudentControllerBlue();
		RController	ctl = new SPlanController();
		MasterSim	sim = new MasterSim(ctl);
